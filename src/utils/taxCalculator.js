...
export function calculateTaxBySlabs(taxableIncome, slabs) {
	...
	for (const slab of slabs) {
		...
		const slabUpperLimit =
			slab.max === Infinity ? taxableIncome : Math.min(slab.max, taxableIncome);
...
export function calculateHRAExemption(
	...
	const exemption2 =
		rentPaid - basicSalary * FORM_CONSTANTS.RENT_DEDUCTION_PERCENTAGE;
...
export function calculateRSUDetails(salaryData) {
	...
	const exchangeRate = salaryData.rsuExchangeRate || 83.0;
...
export function calculateOldRegimeTaxableIncome(salaryData) {
	...
	const salaryAfterStandardDeduction = Math.max(
		0,
		grossSalary - OLD_REGIME_STANDARD_DEDUCTION,
	);
...
export function calculateOldRegimeTax(salaryData) {
	...
	const taxOnRSUIncome =
		rsuDetails.netRSU > 0 && taxableIncome > 0
	...
	const dtaaCredit =
		tax > 0
			? calculateDTAACredit(
					rsuDetails,
					taxOnRSUIncome +
						surcharge * (taxOnRSUIncome / tax) +
						cess * (taxOnRSUIncome / tax),
				)
...
export function calculateNewRegimeTax(salaryData) {
	...
	const rebate =
		taxableIncome <= SECTION_87A_REBATE_LIMIT ? SECTION_87A_REBATE_AMOUNT : 0;
	if (rebate > 0) {
...
export function calculateMonthlyBreakdown(salaryData) {
	...
	const monthlyBasic =
		(salaryData.basicSalary || 0) / TIME_CONSTANTS.MONTHS_IN_YEAR;
	...
	const monthlyProvidentFund =
		((salaryData.basicSalary || 0) * FORM_CONSTANTS.EPF_EMPLOYEE_PERCENTAGE) /
		TIME_CONSTANTS.MONTHS_IN_YEAR;
	...
	const monthlyOldTax =
		oldRegimeResult.totalTax / TIME_CONSTANTS.MONTHS_IN_YEAR;
	const monthlyNewTax =
		newRegimeResult.totalTax / TIME_CONSTANTS.MONTHS_IN_YEAR;
	...
	return months.map((month, index) => {
		...
		const monthlyUSTaxWithheld = hasRSUPayout
			? rsuDetails.usTaxWithheldPerQuarter
			: 0;
		...
		const monthlyGross =
			monthlyBasic + monthlyHRA + monthlyAllowances + monthlyNetRSU;
		...
		const cumulativeGrossSalary =
			cumulativeBasic + cumulativeHRA + cumulativeAllowances + cumulativeNetRSU;
		...
		return {
			...
			// Net salary
			netSalaryOld: Math.round(
				monthlyGross - monthlyOldTax - monthlyProvidentFund,
			),
			netSalaryNew: Math.round(
				monthlyGross - monthlyNewTax - monthlyProvidentFund,
			),
		};
	});
}
...
export function calculateAnnualSummary(salaryData) {
	...
	oldRegimeDeductions += Math.min(
		salaryData.section80C || 0,
		FORM_CONSTANTS.SECTION_80C_LIMIT,
	);
	oldRegimeDeductions += salaryData.section80D || 0;
	oldRegimeDeductions += Math.min(
		salaryData.section24B || 0,
		FORM_CONSTANTS.SECTION_24B_LIMIT,
	);
	...
	return {
		...
		recommendedRegime:
			newRegimeResult.totalTax < oldRegimeResult.totalTax ? "new" : "old",
	};
}
