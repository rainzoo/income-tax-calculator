...
describe('Tax Rules Constants', () => {
  ...
  describe('Consistency Checks', () => {
    it('should have consistent deduction limits', () => {
      expect(FORM_CONSTANTS.SECTION_80C_LIMIT).toBe(DEDUCTION_LIMITS.SECTION_80C)
      expect(FORM_CONSTANTS.SECTION_24B_LIMIT).toBe(DEDUCTION_LIMITS.SECTION_24B)
    })
    ...
    it('should have valid tax slab progressions', () => {
      ...
      for (let i = 1; i < OLD_REGIME_SLABS.length; i++) {
        expect(OLD_REGIME_SLABS[i].rate).toBeGreaterThanOrEqual(OLD_REGIME_SLABS[i - 1].rate)
      }

      for (let i = 1; i < NEW_REGIME_SLABS.length; i++) {
        expect(NEW_REGIME_SLABS[i].rate).toBeGreaterThanOrEqual(NEW_REGIME_SLABS[i - 1].rate)
      }
    })
