...
import { hasErrors, validateForm } from "../utils/validation.js";
...
export default function SalaryInputForm({ onCalculate }) {
	...
	const calculateHRA = useCallback((basicSalary, isMetroCity) => {
		...
		const hraPercentage = isMetroCity
			? FORM_CONSTANTS.HRA_METRO_PERCENTAGE
			: FORM_CONSTANTS.HRA_NON_METRO_PERCENTAGE;
	...
	const handleSubmit = useCallback(
		(e) => {
			...
			const salaryData = {
				...
				rsuExchangeRate:
					parseFloat(formData.rsuExchangeRate) ||
					FORM_CONSTANTS.DEFAULT_USD_EXCHANGE_RATE,
				rsuWithholdingRate:
					parseFloat(formData.rsuWithholdingRate) ||
					FORM_CONSTANTS.DEFAULT_US_TAX_WITHHOLDING,
		...
		<Card sx={{ mb: 4 }} role="main" aria-labelledby="salary-form-title">
			<CardContent sx={{ p: 4 }}>
				...
				<form
					...
					noValidate
					aria-describedby="salary-form-description"
				>
